...
if __name__ == "__main__":
    ...
    parser.add_argument("--environment", type=str, default="TwoArmLift") # Lift TwoArmLift
    parser.add_argument("--robots", nargs="+", type=str, default=["Panda","Panda"], help="Which robot(s) to use in the env") # Panda ["Panda","Panda"]
    parser.add_argument(
        "--config", type=str, default="parallel", help="Specified environment configuration if necessary" # default parallel
    )
    ...
    # parser.add_argument("--device", type=str, default=["joycon"]) # 单臂
    parser.add_argument("--device", type=str, nargs="+",default=["joycon","joycon"])
    ...
    env = suite.make(
        **config,
        ...
        render_camera=None,# "agentview",frontview, None
        ignore_done=True,
    ...
    elif "joycon" in args.device:
        from robosuite.devices.joycon_controller import JoyConController
        device = []
        
        if len(args.device) > 1:
            device_name_list = ['left','right']
        else:
            device_name_list = ['right']
            
        for idx in range(len(args.device)):
            device.append(JoyConController(env=env, device=device_name_list[idx], active_robo=idx))
    else:
    ...
    while True:
        ...
        while True:
            ...
            for idx in range(len(device)):
                # Set active robot
                # print(f'{idx=}, {device[idx].active_robot=}')
                active_robot = env.robots[device[idx].active_robot]

                # Get the newest action
                input_ac_dict = device[idx].input2action()
                ...
                env_action[device[idx].active_robot] = active_robot.create_action_vector(action_dict)
                for gripper_ac in all_prev_gripper_actions[device[idx].active_robot]:
                    all_prev_gripper_actions[device[idx].active_robot][gripper_ac] = action_dict[gripper_ac]
            ...
            if args.max_fr is not None:
                ...
                print(f'action = {env_action}, control rate = {1/elapsed}')
